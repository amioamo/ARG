    ...
    def fit(self, train_data: pd.DataFrame) -> "CTGAN":
       ...
       # print(self._device)
        ...
        for i in range(epochs):
            ...
            if i != 0 and (i % self.SAVE_MODEL_EVERY) == 0:
                print(f'Saving model at epoch {i}')
                torch.save(self._generator.state_dict(), os.path.join(self.model_dir, f'gen_epoch_{i}_{self.seed}.pth'))
                torch.save(self._discriminator.state_dict(), os.path.join(self.model_dir, f'dis_epoch_{i}_{self.seed}.pth'))
                # Save training loss and runtime
                np.save(os.path.join(self.loss_dir, f'epoch_losses_epoch_{i}_{self.seed}.npy'), np.array(epoch_losses))
                np.save(os.path.join(self.time_dir, f'epoch_times_epoch_{i}_{self.seed}.npy'), np.array(epoch_times))
...
def get_generator(
    ...
    class LocalGenerator():
        def __init__(self):
            self.model = CTGAN(model_dir = model_dir, loss_dir=loss_dir ,
                               time_dir = time_dir, epochs=epochs,seed = seed,verbose= False )
...
if __name__ == '__main__':
    ...
    print('Now training for dataset ' + dataset)
    base_dir = "../model/" + dataset + "/ctgan/"
    data_file ="../data/" + dataset + "/real/"
    syn_data_file = "../data/" + dataset + "/syn/ctgan/"
    ...
    for snp_num in args.snp_nums:
        ...
        for train_raito in args.train_ratios:
            ...
            syn_dir = syn_data_file + str(snp_num) + '/' + str(train_raito)
            ...
            sample_dir = os.path.join(syn_data_file, 'ctgan')
            os.makedirs(model_dir, exist_ok=True)
            os.makedirs(loss_dir, exist_ok=True)
            os.makedirs(time_dir, exist_ok=True)
            os.makedirs(sample_dir, exist_ok=True)

            # for i_iter, seed in enumerate([0, 42, 50, 100, 245]):

            for i_iter, seed in enumerate(args.seeds):
                print(f'now running for {snp_num}_{train_raito}_{i_iter}')
                data = np.load(data_file  + str(snp_num) + '/' + \
                                           str(train_raito)+ '/' + 'train_'+str(seed)+'.npy')

                df = pd.DataFrame(data)
                df.columns = [str(_) for _ in range(data.shape[1])]
                ...
                generator.fit(df)
                synth_set = generator.generate(SYN_SIZE)
                with open(os.path.join(syn_dir, f'sample_data_epoch_{EPOCHS}_{seed}.pkl'), 'wb') as file:
                    pickle.dump(synth_set, file)
